...
export function generateManagedContent() {
  ...
  lines.push('');
  lines.push('You MUST track Acceptance Criteria completion in **two** ways:');
  lines.push('');
  lines.push('### 1. Incremental Tracking (during execution)');
  lines.push('');
  lines.push('As you complete EACH Acceptance Criteria individually, emit immediately:');
  lines.push('');
  lines.push('```');
  lines.push('[AC_COMPLETE] <exact AC text without the "- [ ] " prefix>');
  lines.push('```');
  lines.push('');
  lines.push('**Example:** if AC is `- [ ] Login page renders correctly`');
  lines.push('Emit: `[AC_COMPLETE] Login page renders correctly`');
  lines.push('');
  lines.push('Emit this marker IMMEDIATELY after completing each AC, before moving to the next.');
  lines.push('');
  lines.push('### 2. Final Tracking (in JSON response)');
  lines.push('');
  lines.push('Your JSON response MUST include a `completed_acs` field (see Response Format below).');
  lines.push('This field is **MANDATORY**, not optional.');
  lines.push('List ALL completed Acceptance Criteria exactly as they appear (without `- [ ] ` prefix).');
  lines.push('');
  lines.push('If you complete ANY Acceptance Criteria but fail to include them in `completed_acs`,');
  lines.push('your response will be **REJECTED** and you will be asked to retry.');
  lines.push('');

  // Response Contract
  lines.push('## Response Format (MANDATORY)');
  lines.push('');
  lines.push('You MUST respond with a valid JSON object in a **single line**.');
  lines.push('');
  ...
  lines.push('  "completed_acs": ["First AC text", "Second AC text"]');
  lines.push('}');
  lines.push('```');
  lines.push('');
  ...
  lines.push('| `completed_acs` | **MANDATORY** — Array of completed Acceptance Criteria texts. |');
  lines.push('');
  lines.push('**WARNING:** If you complete ANY Acceptance Criteria but omit them from `completed_acs`,');
  lines.push('your response will be **REJECTED**. This field is NOT optional.');
  lines.push('');
  lines.push('**Example valid response:**');
  lines.push('');
  lines.push('```json');
  lines.push('{"status":"done","summary":"Implemented login page with form validation","notes":"Used React Hook Form for validation","files":["src/pages/Login.tsx","src/components/LoginForm.tsx"],"tests":"5 tests passing","completed_acs":["Login page renders correctly","Form validates email format","Error messages are displayed"]}');
  lines.push('```');
  lines.push('');
  ...
  lines.push('- Track EACH completed AC using `[AC_COMPLETE]` markers (see above).');
